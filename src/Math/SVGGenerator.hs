...
tangentLine p1 p2 c d = begArrowHead # fc c <> shaft # lc c # d # lw veryThin <> endArrowHead # fc c
  where
    v = p2 .-. p1
    shaft     = p1 ~~ p2
    arrowHead = eqTriangle 0.1
...
dashPatternFrom s = case s of
  "solid" -> dashing [] ZeroM
  "dashed" -> dashing [verySmall, tiny] ZeroM
  "dotted" -> dashing [medium, medium] ZeroM
  _        -> dashing [] ZeroM
